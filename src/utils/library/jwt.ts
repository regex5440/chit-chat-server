...
function generateNewToken(data = {}, type: "login" | "signup" = "login") {
  ...
  return jwt.sign(data, type === "login" ? process.env.TOKEN_KEY : process.env.SIGNUP_TOKEN_KEY, {
...
async function validateToken<T extends SessionType>(token: string, type: T): ValidTokenResponse<T> {
  return new Promise((resolve, reject) => {
    try {
      ...
      jwt.verify(token, type === "login" ? process.env.TOKEN_KEY : process.env.SIGNUP_TOKEN_KEY, async function (err, data) {
        if (err) {
          reject(err);
        } else if (type === "login") {
